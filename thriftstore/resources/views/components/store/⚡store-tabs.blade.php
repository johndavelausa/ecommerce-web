...
new class extends Component
{
    ...
    public function getCategoriesWithCountsProperty(): array
    {
        ...
        return $rows->mapWithKeys(fn ($r) => [$r->category => (int) $r->count])->all();
    }
    ...
    {
        return Review::query()
            ->whereHas('product', fn ($q) => $q->where('seller_id', $this->sellerId))
...
<div class="max-w-5xl mx-auto sm:px-6 lg:px-8 mt-6">
            ...
            @if($products->count())
                ...
                <div class="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    @foreach($products as $product)
                        ...
                        @endphp
                        <div class="border border-gray-200 rounded-lg overflow-hidden flex flex-col relative {{ $isOutOfStock ? 'opacity-75' : '' }}">
                            @if($isOutOfStock)
                                <div class="absolute inset-0 bg-gray-100/70 z-10 flex items-center justify-center pointer-events-none">
                                    <span class="bg-gray-700 text-white text-xs font-semibold px-3 py-1.5 rounded-full uppercase">Out of Stock</span>
                                </div>
                            @endif
                            <div class="relative h-44 bg-gray-100">
                                ...
                                <button type="button" wire:click="toggleWishlist({{ $product->id }})" class="absolute top-2 right-2 inline-flex items-center justify-center w-7 h-7 rounded-full bg-white/80 hover:bg-white text-xs z-20">
                                    @if(isset($wishlist[$product->id]))<span class="text-rose-500">♥</span>@else<span class="text-gray-400">♡</span>@endif
                                </button>
                                @if($product->sale_price)
                                    <span class="absolute top-2 left-2 bg-rose-600 text-white text-2xs px-2 py-0.5 rounded-full uppercase tracking-wide z-20">Sale</span>
                                @endif
                                @if($isLowStock)
                                    <span class="absolute bottom-2 left-2 bg-amber-500 text-white text-2xs px-2 py-0.5 rounded z-20">Low Stock</span>
                                @endif
                            </div>
                            <div class="flex-1 flex flex-col p-3 space-y-1">
                                <a href="{{ route('product.show', $product->id) }}" class="text-sm font-semibold text-gray-900 line-clamp-2 hover:text-indigo-600">{{ $product->name }}</a>
                                <div class="text-xs flex flex-wrap gap-1 items-center">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded font-medium bg-indigo-100 text-indigo-800">{{ \App\Models\Product::conditionOptions()[$product->condition] ?? ucfirst(str_replace('_', ' ', $product->condition ?? 'Good')) }}</span>
                                    @if($product->size_variant)
                                        <span class="inline-flex items-center px-2 py-0.5 rounded font-medium bg-gray-100 text-gray-700">{{ (\App\Models\Product::sizeVariantOptions())[$product->size_variant] ?? $product->size_variant }}</span>
                                    @endif
                                    @if($product->seller?->is_verified ?? false)
                                        <span class="inline-flex items-center text-blue-600" title="Verified seller">✓</span>
                                    @endif
                                </div>
                                <div class="mt-1 flex items-baseline gap-2">
                                    @if($product->sale_price)
                                        <span class="text-base font-semibold text-rose-600">₱{{ number_format($product->sale_price, 2) }}</span>
                            ...
                            <div class="px-3 pb-3">
                                @if($product->stock > 0)
                                    <button type="button" wire:click="addToCart({{ $product->id }})" class="w-full inline-flex justify-center items-center px-3 py-2 bg-indigo-600 border border-indigo-600 rounded-md text-xs font-semibold text-white uppercase tracking-widest shadow-sm hover:bg-indigo-500">Add to cart</button>
                                @else
                                    <button type="button" disabled class="w-full inline-flex justify-center items-center px-3 py-2 bg-gray-300 border border-gray-300 rounded-md text-xs font-semibold text-gray-600 uppercase tracking-widest cursor-not-allowed">Add to cart</button>
                                @endif
                            </div>
                        </div>
                    @endforeach
    ...
    @if($activeTab === 'categories')
        <div class="bg-white rounded-lg shadow p-4 sm:p-6 mt-4">
            @php($categoriesWithCounts = $this->categoriesWithCounts)
            <div class="flex flex-wrap gap-2 mb-4">
                ...
                @foreach($categoriesWithCounts as $cat => $count)
                    <button type="button"
                            wire:click="setCategory({{ json_encode($cat) }})"
    ...
    @if($activeTab === 'reviews')
        <div class="bg-white rounded-lg shadow p-4 sm:p-6 mt-4">
            @php($summary = $this->storeReviewsSummary)
            @if($summary['total'] > 0)
                <div class="flex flex-wrap gap-8 mb-6 pb-6 border-b border-gray-200">
                    ...
                    <div class="text-sm text-gray-600">{{ $summary['total'] }} {{ $summary['total'] === 1 ? 'review' : 'reviews' }}</div>
                        ...
                        @foreach([5, 4, 3, 2, 1] as $stars)
                            <div class="flex items-center gap-2 text-sm">
                                ...
                                <div class="flex-1 h-3 bg-gray-200 rounded overflow-hidden">
                                    <div class="h-full bg-amber-400 rounded" style="width: {{ $summary['maxBar'] > 0 ? round(100 * ($summary['breakdown'][$stars] ?? 0) / $summary['maxBar']) : 0 }}%"></div>
                                </div>
                    ...
                    @foreach($reviews as $review)
                        <div class="border border-gray-200 rounded-lg p-4">
                            ...
                            <div class="mt-2 text-sm">
                                ...
                                <a href="{{ route('product.show', $review->product_id) }}" class="text-indigo-600 hover:underline">{{ $review->product->name ?? '—' }}</a>
                            </div>
